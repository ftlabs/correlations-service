...
function send_metrics() {
	...
	/*
		Create a connection to the graphite server on port 2003.
		A connection is created each time metrics will be sent and closed after the metrics have been send.
	*/
	...
	load = os.loadavg();
	const mem_usage = Math.floor((os.totalmem() - os.freemem())/os.totalmem() *10000)/100;
	const cpu_load = Math.floor( (load[0] / os.cpus().length) * 10000) /100;
  ...
  //OS metrics
  var os_metrics = METRIC_PREFIX + "os_metrics.mem_usage" + " " + parseFloat(mem_usage) + " " + Math.floor(Date.now() / 1000) + "\n";
  var os_metrics = os_metrics + METRIC_PREFIX + "os_metrics.cpu_load" + " " + parseFloat(cpu_load) + " " + Math.floor(Date.now() / 1000) + "\n";

	// Group all the metrics
	metrics_to_send = metric_line1 + metric_line2 + metric_line3 + os_metrics;

//  console.log(metrics_to_send);
